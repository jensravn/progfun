...
class ListsSuite {
  ...
  @Test def `max of a list with single element (10pts)`(): Unit = {
    assert(max(List(2)) == 2)
  }

  @Test def `max of a list with biggest number first (10pts)`(): Unit = {
    assert(max(List(5, 3, 1)) == 5)
  }

  @Test def `max of a list with biggest number last (10pts)`(): Unit = {
    assert(max(List(1, 3, 5)) == 5)
  }

  @Test def `max of a big list (10pts)`(): Unit = {
    max(List.range(0, 1000000))
  }
